...
t_start = tab_off.t(1)-.5e-6;
t_stop = tab_off.t(numel(tab_off.t))+.5e-6;
if(!exist("numsteps"))
	numsteps = 1000;
endif
...
plot(mer.tus, mer.vc/10, "k", mer.tus, mer.ic*2.5, "k;meranie;", sim_off.tus, sim_off.vc/10, "k--", sim_off.tus, sim_off.ic*2.5, "k--", sim_off.tus, sim_off.gce/2*5, "k--;simulácia;");
legend("boxoff");
grid;
...
text(t_start*1e6+.1, sim_off.ic(1)*2.5+2, '$i_{C}$', 'interpreter', 'latex');
system("mkdir plots");
...
%%% turn on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
t_start = tab_on.t(1)-.5e-6;
t_stop = tab_on.t(numel(tab_on.t))+.5e-6;
if(!exist("numsteps"))
	numsteps = 1000;
endif
timestep = (t_stop-t_start)/numsteps;
...
plot(mer.tus, mer.vc/10, "k", mer.tus, mer.ic*2.5, "k;meranie;", sim_on.tus, sim_on.vc/10, "k--", sim_on.tus, sim_on.ic*2.5, "k--", sim_on.tus, sim_on.gce/2*5, "k--;simulácia;");
legend("boxoff");
grid;
xlim(1e6*[t_start t_stop]);
ylim([-2 50]);
...
text(t_start*1e6+.1, sim_on.ic(1)*2.5+2, '$i_{C}$', 'interpreter', 'latex');
system("mkdir plots");
