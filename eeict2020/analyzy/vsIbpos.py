...
#loadpath = '../mer/kolektor_v_lufte/vs_Ibpos/'
loadpath = '../../../dis/merania/mer_BUX/mer/kolektor_v_lufte/vs_Ibpos/'
#filename = '300V_bb'
filenames = ['ibneg1', 'ibneg2']
#filenames = ['ibneg2']
savefilename = "vsIbpos"
...
ax11.set_xlabel("Base Drive Current (A)")
ax11.set_ylabel(r'Deflated Charge ($\mu C$)')
ax12.set_xlabel("Base Drive Current (A)")
...
#for i in range(1,2):
    ...
    iiibpos = iiiibpos[i][1:]
    sort_index = np.argsort(iiibpos)
    sort_index = range(len(iiibpos))
    # scaling na mikro
    qqq=np.array(qqqq[i][1:])[sort_index]/1e-6
    ttt = np.array(tttt[i][1:])[sort_index]

    iiibpos=np.abs(iiibpos)
    xs, ys = myspline_w(iiibpos[:], qqq[:], wwextra=.15, k=4)
    ax11.plot(xs, ys, 'k')
    ax11.plot(iiibpos, qqq, 'o', color=colors[i], label=r'$I_{B-}=$'+f'{Ibneg}A')

    xs, ys = myspline_w(iiibpos[:], ttt[:], wwextra=.5)
    ax12.plot(xs, ys, 'k')
    ax12.plot(iiibpos, ttt, 'o', color=colors[i], label=r'$I_{B-}=$'+f'{Ibneg}A')
