...
for aaa in aaaa:
    for aa in aaa[::-1]:
        ...
        ax1.fill_between(aa.tus[aa.tr1:aa.tr3]-tshift, aa.ch1[aa.tr1:aa.tr3], color=fillcolors[i], label=r'$Q_{defl.}, I_{B+}=$' + f'{round(aa.ibpos,2)}A')
        ax1.plot(aa.tus-tshift, aa.ch1, 'k')
        fig.tight_layout() # a kuknut aj constrained_layout
        if i is 0:
            ax1.hlines([0, aa.ibpos], 0, 1.5)
            ax1.annotate(s="", xy=(1, 0), xytext=(1, aa.ibpos), arrowprops=dict(arrowstyle='<->'))
            ax1.text(1.2, aa.ibpos/2, r"$I_{B+}$")
